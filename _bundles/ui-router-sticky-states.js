...
var notInArray = function (arr) { return function (item) { return !core.inArray(arr, item); }; };
...
var StickyStatesPlugin = /** @class */ (function (_super) {
    ...
    function StickyStatesPlugin(router) {
        ...
        _this._getInactive = function (node) {
            return node && core.find(_this._inactives, function (n) { return n.state === node.state; });
        };
    ...
    StickyStatesPlugin.prototype._calculateStickyTreeChanges = function (trans) {
        ...
        var inactiveFromPath = tc.retained.concat(tc.entering.map(function (node) { return _this._getInactive(node) || null; })).filter(core.identity);
        ...
        trans.onSuccess({}, function () {
            tc.exiting.forEach(core.removeFrom(_this._inactives));
            tc.entering.forEach(core.removeFrom(_this._inactives));
            tc.reactivating.forEach(core.removeFrom(_this._inactives));
        ...
        exitSticky.filter(notInArray(tc.exiting)).forEach(core.pushTo(tc.exiting));
        // Also process the active sticky states that are about to be inactivated, but should be exited
        exitSticky.filter(core.inArray(tc.inactivating)).forEach(core.removeFrom(tc.inactivating));
        return tc;
    };
